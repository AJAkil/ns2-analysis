...
for {set i 0} {$i < $val(nn) } {incr i} {
    ...
    $node($i) set X_ [expr int(rand() * $area)+1]
    $node($i) set Y_ [expr int(rand() * $area)+1]
    $node($i) set Z_ 0

    set destX [expr int(rand() * $area)]
    set destY [expr int(rand() * $area)]
    set velocity [expr int(rand() * 5) + 1]

    $ns at 1.0 "$node($i) setdest $destX $destY $velocity"
